...
destructor, static member functions, friend class, this pointer, inline code and dynamic memory allocation
...
Student::Student(  char* name,  char* roll_no, char* cls, char* div, char* dob, char* blood_grp, char*  address,long long int tele_no,int license_no){
	//assigning the exact amount of memory(by finding len of str) required to store the var
	int length;
		length = strlen(name);
...
//new - The new operator denotes a request for memory allocation on the Free Store. If sufficient memory is available, new operator initializes the memory and returns the address of the newly allocated and initialized memory to the pointer variable.
