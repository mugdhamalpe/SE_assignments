...
public:
	Node(bool isBooked = false, int start_time = 0, int end_time = 0,
			int min_duration = 0, int max_duration = 60) {
...
public:
	...
	void DisplaySlots() {
		if (head != nullptr) //not an empty list
				{
			Node *temp = head;
			cout << "****Display of booked slots****" << endl;
			cout << left << setw(15) << "Start Time" << left << setw(15)
					<< "End Time" << left << setw(15) << "Max Duration" << left
					<< setw(15) << "Min Duration" << endl;
			while (temp != nullptr) {
				cout << left << setw(15) << temp->start_time << left << setw(15)
						<< temp->end_time << left << setw(15)
						<< temp->max_duration << left << setw(15)
						<< temp->min_duration << endl;
				temp = temp->next;
			}
	...
	void BookAppointment() {
		...
		cin >> startTime;
		Node *temp = head;
		while (temp != nullptr) //traverse till end of list
		{
	...
	void SortStartTime() {
		for (int i = 0; i < noOfSlots - 1; i++) {
			Node *temp = head;
			while (temp->next != nullptr) //traverse till end of linked list
			{
	...
	// void insertionSort()
	// {
	//     Node *current = head; //assigning current to head

	//     while(current != nullptr) //traverse till end of list
	//     {
	//         Node *next = current->next; //next pointer of node type which is pointing to next of current
	//         insert(current);
	//         current = next;
	//     }
	//     head = sorted; //assign head to sorted which is nullptr
	// }
};
...
int main() {
	...
	while (repeat) {
		...
		case 3:
			today.SortStartTime();
			cout << "Sucessfully sorted appointments based on start time..."
					<< endl;
			today.DisplaySlots();
			break;
		...
		case 4:
			...
			// case 5:
			//     today.insertionSort();
			//     cout<<"Sucessfully sorted appointments using Insertion sort..."<<endl;
			//     today.DisplaySlots();
			//     break;
